...
from modules.defaults import get_input, get_default_parameters
import pickle
from modules.unpack_mat_data import load_mat_data
...
def plot_activity(timecourse, dt, file_base):
    ...
    for i in range(ax.size):
        ...
        ax[ax_i].set_ylabel('Response')
        if 'attention' in key:
            ax[ax_i].set_ylim([-0.55, 0.55])
            ax[ax_i].set_xlabel('Time [sec]')
        else:
...
if __name__ == '__main__':
    ...
    for attention_weights in all_attention_weights:
        ...
        for pop_params in params:
            pop_params['total_duration'] = 30000
            pop_params['dt'] = 1.
        #

        # initialize the network
        network = Network(1., *params)

        # overwrite attention weights
        network = overwrite_weight_attention(network, new_wa=attention_weights)
        ...
        timecourse = network.simulate(sensory_input, attention_simulate_external_drive=[0, 0])

        file_base = r'simulation_data_attention_weights\simulation_attention_weights_[%0.3f,%0.3f]' % \
                    (attention_weights[0], attention_weights[1])

        # comparision_python_matlab_plot(timecourse)
        plot_activity(timecourse, dt, file_base)

        # save the results
        save_path = open(file_base + '.pkl', 'wb')
